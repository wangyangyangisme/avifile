...
static inline REAL layer3twopow2(int scale,int preflag,int pretab_offset,int l)
{
    int index=l;
...
static inline REAL layer3twopow2_1(int a,int b,int c)
{
    return POW2_1[a][b][c];
}
...
void Mpegtoraw::layer3initialize(void)
{
 ...
 //  register int i;
  ...
  // recompute TABLE for AUDIO GAIN
...
bool Mpegtoraw::layer3getsideinfo(void)
{
  ...
  for(int gr=0;gr<2;gr++)
    for(int ch=0;;ch++)
    {
      ...
      memset(gi, 0, sizeof(*gi)); // ensure ths structure will be clear
...
void Mpegtoraw::huffmandecoder_1(const HUFFMANCODETABLE *h,int *x,int *y)
{
...
void Mpegtoraw::huffmandecoder_2(const HUFFMANCODETABLE *h,
...
static SFBANDINDEX sfBandIndextable[3][3]=
{
  ...
  {{{0,6,12,18,24,30,36,44,54,66,80,96,116,140,168,200,238,284,336,396,464,522,576},
   ...
   {{0,6,12,18,24,30,36,44,54,66,80,96,116,140,168,200,238,284,336,396,464,522,576},
    {0,4,8,12,18,26,36,48,62,80,104,134,174,192}},
   {{0,12,24,36,48,60,72,88,108,132,160,192,232,280,336,400,476,566,568,570,572,576},
...
void Mpegtoraw::layer3huffmandecode(int ch,int gr,int out[SBLIMIT][SSLIMIT])
{
  ...
  {          /* Find region boundary for long block case. */
    region1Start=sfBandIndextable[mpeg25?2:version][frequency].l[gi->region0_count+1];
    region2Start=sfBandIndextable[mpeg25?2:version][frequency].l[gi->region0_count+
						        gi->region1_count+2];
  }
				       ...
				       REAL out[SBLIMIT][SSLIMIT])
{
  ...
  REAL globalgain=POW2[gi->global_gain] * gain_add / 8;
  REAL *TO_FOUR_THIRDS=TO_FOUR_THIRDSTABLE+FOURTHIRDSTABLENUMBER;
...
void Mpegtoraw::layer3fixtostereo(int gr,REAL in[2][SBLIMIT][SSLIMIT])
{
  ...
  if(i_stereo)
  {
	...
	for(;sfb<21;sfb++)
	  ...
	  t=is_pos[tt];
	  //printf("TT  %d   K  %d   T %d  I %d  %d\n", tt, k, t, i, ARRAYSIZE);
	  if(t!=7)
	  {
	      RATIOS r=is_ratio[tt];

	      for(;k>0 && i < ARRAYSIZE;k--,i++){
		  is_pos[i]=t; is_ratio[i]=r;
	      }
	  }
	  else
