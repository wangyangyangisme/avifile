 ...
 *
 */ 
unsigned long XA_ADecode_GSMM_PCMxM
(
//XA_SND *snd_hdr,
long byte_cnt,
long samp_cnt,
...
//  unsigned char *ibuf;
  int ocnt=0;
  int flag=0;
  unsigned long inc_cnt=0; 
  long dataL, dataR;
  unsigned long out_cnt = 320;
//  printf("Received %d bytes in %d samples\n", byte_cnt, samp_cnt);
  //spec		= snd_hdr->spec;	
  // misc flags
  // 0x80: AVI vs QT
  // 0x8: ulaw translation
  // 0x4: 8-bit sound
  // 0x2: signed/unsigned 
  // 0x1: big-endian
//  inc		= snd_hdr->inc;		
...
//	printf("Read %d bytes into decoder\n", icnt);
//	else
//           XA_GSM_Decoder(&gsm_state, ibuf, gsm_buf, &icnt, &out_cnt);
	...
	/*** Decode Sample and increment byte_cnt ***/
      if (flag < out_cnt)  
          dataL  = (unsigned long)gsm_buf[ flag ];
      else
      /* ERROR */ dataL = 0;

      flag++; if (flag >= out_cnt) flag = 0;
//      inc_cnt += inc;
//    }
...
//      { unsigned char d0 = ((unsigned long)(dataL) >> 8) & 0xff;
//        if (spec & 8)    *obuf++ = xa_sign_2_ulaw[ (unsigned long)(d0) ];
//        else             *obuf++ = (spec & 2)?(d0 ^ 0x80):(d0);
//      }
//      else
      { unsigned char d1,d0; d1 = (dataL>>8) & 0xff; d0 = dataL & 0xff;
//        if (spec & 0x02)
//	 d1 ^= 0x80;
//        if (spec & 0x01)
//	{
