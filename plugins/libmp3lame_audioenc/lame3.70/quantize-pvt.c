...
void quantize_xrpow(FLOAT8 xr[576], int ix[576], gr_info *cod_info) {
...
#if defined (USE_GNUC_ASM) 
  {
      ...
      __asm__ __volatile__(
        ...
        "fld" F8type " " F8size "+" F8size "+" F8size "(%1)\n\t"
        ...
        "addl $" F8size "+" F8size "+" F8size "+" F8size ", %1\n\t"
        "addl $16, %3\n\t"
        ...
        "pop %%ebx\n\n"
	: /* no outputs */
        : "t" (istep), "r" (xr), "r" (adj43asm), "r" (ix), "r" (576 / 4), "m" (rx[0])
        : "%eax", "memory", "cc"
      );
  }
#elif defined (USE_MSC_ASM)
  ...
  const FLOAT8 istep = IPOW20(cod_info->global_gain);
...
#if defined(USE_GNUC_ASM)
   {
      __asm__ __volatile__ (
        ...
        "fld" F8type " " F8size "+" F8size "+" F8size "(%3)\n\t"

        "addl $" F8size "+" F8size "+" F8size "+" F8size ", %3\n\t"
        "addl $16, %4\n\t"
