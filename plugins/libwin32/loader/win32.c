...
static void WINAPI expEnterCriticalSection(CRITICAL_SECTION* c)
{
    ...
    if (!cs)
    {
	dbgprintf("Win32 Warning: entered uninitialized Critical Section!\n");
...
{
    tls_t* t = my_mreq(sizeof(tls_t), 0);
    dbgprintf("TlsAlloc() => 0x%x\n", t);
    if (t)
	t->value = 0; /* XXX For Divx.dll */
    return t;
}
...
static void* WINAPI expTlsGetValue(void* idx)
{
    static int i = 0;
    tls_t* t = (tls_t*) idx;
    void* result = (t) ? t->value : 0;
    if (i > 156)
        trapbug();
    dbgprintf("TlsGetValue(index 0x%x) => 0x%x %d\n", index, result, i++);
    return result;
}
...
static const void* WINAPI expGetProcAddress(HMODULE mod, char* name)
{
    const void* result;
    switch(mod){
				       ...
				       long dwClsContext, const GUID* riid, void** ppv)
{
	...
	if(!memcmp(rclsid, &com_object_table[i].clsid, sizeof(GUID)))
	    ...
	    return com_object_table[i].GetClassObject(rclsid, riid, ppv);
...
{
    dbgprintf("GetCurrentProcessId() => %d\n", getpid());
...
const void* LookupExternal(const char* library, int ordinal)
{
    int i,j;
    if(library==0)
    {
...
const void* LookupExternalByName(const char* library, const char* name)
{
    char* answ;
    int i,j;
    //   return (void*)ext_unknown;
